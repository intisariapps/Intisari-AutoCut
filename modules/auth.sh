...
# VERSION: V4.8 (Pure Random Persistent ID & Full Integration) AUTH=4.7
...
cek_lisensi_online() {
    ...
    if [[ "$input_key" == *"TRIAL"* ]]; then
        ...
        # [FIX] TAHUN EXPIRED OTOMATIS MENGIKUTI TAHUN SERVER/SAAT INI
        local bulan_exp="${input_id:0:2}"; local hari_exp="${input_id:2:2}"; local tahun_exp=$(date +%Y)
